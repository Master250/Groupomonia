...
class PostsManager {
    constructor() {
        console.log('PostsManager connecté')
    }
    
//POSTS

    getAllPosts(){
        let sql = "SELECT posts.id, posts.userId, posts.title, posts.content, posts.imageUrl, DATE_FORMAT(DATE(posts.date), '%d/%m/%Y') AS date, TIME(posts.date) AS time, posts.likes, users.lastName, users.firstName, users.avatar FROM posts JOIN users ON posts.userId = users.id ORDER BY posts.date DESC";
        ...
        return new Promise((resolve, reject) =>{
            connectdb.query(sql1, function (err, result, fields){
                ...
                if(sqlInserts2[4] == result[0].userId){
                    let sql2 = 'UPDATE posts SET title = ?, content = ?, imageUrl = ? WHERE id = ? AND userId = ?';
...
// COMMENTS
    ...
    getComments(sqlInserts){
        let sql = "SELECT comments.comContent, DATE_FORMAT(comments.date, '%d/%m/%Y à %H:%i:%s') AS date, comments.id, comments.userId, users.firstName, users.lastName, users.avatar FROM comments JOIN users on comments.userId = users.id WHERE postId = ? ORDER BY date";
        sql = mysql.format(sql, sqlInserts);
        return new Promise((resolve) =>{
            connectdb.query(sql, function (err, result, fields){
                if (err) throw err;
